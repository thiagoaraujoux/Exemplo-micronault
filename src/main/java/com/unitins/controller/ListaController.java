...
public class ListaController {
    ...
    @ApiResponse(responseCode = "200", description = "Lista de listas retornada com sucesso.",
                    content = @Content(mediaType = "application/json",
                                        schema = @Schema(type = "array", implementation = Lista.class)))
    ...
    @ApiResponse(responseCode = "404", description = "Lista não encontrada para o ID especificado.",
                    content = @Content(mediaType = "application/json",
                                        schema = @Schema(implementation = ApiResponseMessage.class)))
    @ApiResponse(responseCode = "500", description = "Erro interno do servidor ao tentar buscar a lista.",
                    content = @Content(mediaType = "application/json",
                                        schema = @Schema(implementation = ApiResponseMessage.class)))
     ...
     * POST /api/listas
     * Cria uma nova lista.
     * Returns the created Lista on success (201 Created), or an ApiResponseMessage on error (500 Internal Server Error).
     */
    @Post(consumes = MediaType.APPLICATION_JSON, produces = MediaType.APPLICATION_JSON)
    @Operation(summary = "Criar nova lista",
                description = "Cria uma nova lista com os dados fornecidos no corpo da requisição.")
    ...
    @ApiResponse(responseCode = "400", description = "Requisição inválida (dados ausentes ou mal formatados), validação falhou.",
                    content = @Content(mediaType = "application/json",
                                        schema = @Schema(implementation = ApiResponseMessage.class)))
    @ApiResponse(responseCode = "500", description = "Erro interno do servidor ao tentar criar a lista.",
                    content = @Content(mediaType = "application/json",
                                        schema = @Schema(implementation = ApiResponseMessage.class)))
     ...
     * Returns the updated Lista on success (200 OK), or an ApiResponseMessage on error (404 Not Found or 500 Internal Server Error).
     */
    @Put("/{id}")
    @Operation(summary = "Atualizar lista existente",
                description = "Atualiza os dados de uma lista existente com base no ID.")
    ...
    @ApiResponse(responseCode = "404", description = "Lista não encontrada para o ID especificado.",
                    content = @Content(mediaType = "application/json",
                                        schema = @Schema(implementation = ApiResponseMessage.class)))
    @ApiResponse(responseCode = "500", description = "Erro interno do servidor ao tentar atualizar a lista.",
                    content = @Content(mediaType = "application/json",
                                        schema = @Schema(implementation = ApiResponseMessage.class)))
    public HttpResponse<?> atualizar(@PathVariable Long id, @Body @Valid Lista lista) {
        try {
            ...
            return HttpResponse.notFound(ApiResponseMessage.error("Lista não encontrada para atualização."));
        } catch (Exception e) {
    ...
    @Operation(summary = "Deletar lista",
                description = "Remove a lista com o ID especificado do sistema.")
    @ApiResponse(responseCode = "204", description = "Lista deletada com sucesso (Nenhum conteúdo para retornar).")
    @ApiResponse(responseCode = "404", description = "Lista não encontrada para o ID especificado (já excluída ou nunca existiu).",
                    content = @Content(mediaType = "application/json",
                                        schema = @Schema(implementation = ApiResponseMessage.class)))
    @ApiResponse(responseCode = "500", description = "Erro interno do servidor ao tentar deletar a lista.",
                    content = @Content(mediaType = "application/json",
                                        schema = @Schema(implementation = ApiResponseMessage.class)))
